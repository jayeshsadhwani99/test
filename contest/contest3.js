...
async function fetchData() {
  let res = await fetch(
    "https://api.ipgeolocation.io/ipgeo?apiKey=0edbf436ee42417496fcf0da67df561e"
  );
  let data = await res.json();
  if (data) {
    let weatherRes = await fetch(
      `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${data.latitude},${data.longitude}?unitGroup=us&key=VKB9DM35UV78DW2ELUQEC3FN9`
    );
